#!/bin/bash
# Generate the ChickenBlocker apt browser-install-block preferences file.
#
# Writes the apt preferences pin (Pin-Priority: -1) to stdout, blocking
# install/upgrade of every browser EXCEPT the ones ChickenBlocker itself manages.
#
...
#   ChickenBlocker; chromium-browser is an unwanted browser. Excluding by
...
# ChickenBlocker-update runner) is responsible for clearing/setting +i around the
# write, because only the cap-endowed context can clear +i post-cap-drop.
#
# Usage:
#   ./gen-browser-pin.sh > /etc/apt/preferences.d/99-ChickenBlocker-no-browsers
set -euo pipefail

# Browsers ChickenBlocker MANAGES - excluded from the pin so they keep receiving
# security upgrades. Add a managed browser here when ChickenBlocker starts managing it.
...
cat <<EOF
# Added by ChickenBlocker: block install/upgrade of non-managed browsers via apt.
# Regenerated on every apparmor/install.sh run AND every 'make update' (via the
# cap-endowed ChickenBlocker-update service). ChickenBlocker-MANAGED browsers
