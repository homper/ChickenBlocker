#!/bin/bash
# Install the ChickenBlocker protection (filesystem immutable + AppArmor).
# Run as root, e.g.:  sudo ./install.sh
#
# This blocks `systemctl disable ChickenBlocker` in a ChickenBlocker-SPECIFIC way (does
# NOT affect other units) by combining two layers:
#
#   1. Filesystem immutable DIRECTORY: ChickenBlocker.service is
#      WantedBy=ChickenBlocker.target, and ChickenBlocker.target is
#      WantedBy=multi-user.target. We `chattr +i` the
#      ChickenBlocker.target.wants/ DIRECTORY so the kernel refuses to unlink any
#      entry inside it. When `systemctl disable ChickenBlocker` tries to unlink the
#      ChickenBlocker.service enable symlink from ChickenBlocker.target.wants/, it fails
...
#      ChickenBlocker-specific mechanism that blocks the unlink.
#
#      Residual bypass (intentional): the TOP-level enable symlink
#      multi-user.target.wants/ChickenBlocker.target lives in the SHARED
#      multi-user.target.wants/ dir, which we do NOT make immutable. So
#      `systemctl disable ChickenBlocker.target` still succeeds and prevents
#      boot-time start (the already-running persistent instance is unaffected).
...
#      immutable flags on the ChickenBlocker files / the ChickenBlocker.target.wants/
...
# chattr profile denies opening the ChickenBlocker paths for chattr, including +i).
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
AA_DIR="/etc/apparmor.d"

UNIT=/etc/systemd/system/ChickenBlocker.service
TARGET=/etc/systemd/system/ChickenBlocker.target
TARGET_WANTS_DIR=/etc/systemd/system/ChickenBlocker.target.wants
SERVICE_WANTS_LINK="$TARGET_WANTS_DIR/ChickenBlocker.service"
MU_TARGET_LINK=/etc/systemd/system/multi-user.target.wants/ChickenBlocker.target
# Obsolete wants entry from the OLD (broken) scheme: a hardlink at
# multi-user.target.wants/ChickenBlocker.service. The new scheme does not use it.
OLD_MU_SERVICE_LINK=/etc/systemd/system/multi-user.target.wants/ChickenBlocker.service
ENV=/etc/default/ChickenBlocker
...
# vivaldi-stable, managed by ChickenBlocker itself) are EXCLUDED from the pin at
...
# profile is enforced, `chattr -i`/`chattr +i` on ChickenBlocker-protected +i files is
# DENIED (chattr is AppArmor-confined + the BPF gate only allows comm
# "ChickenBlockUpd" to clear +i). So we try chattr first (works on a FIRST
# install before the gate/profile load, and on non-protected paths), then fall
# back to ChickenBlockUpd (raw FS_IOC_SETFLAGS ioctl; NOT AppArmor-confined; the
# BPF gate allows it because comm == "ChickenBlockUpd"). ChickenBlockUpd needs
...
cb_attr() {
  ...
  if [ -x "$REPO_ROOT/bpf/ChickenBlockUpd" ]; then
    "$REPO_ROOT/bpf/ChickenBlockUpd" "$mode" "$f" && return 0
    echo "ERROR: ChickenBlockUpd $mode $f failed" >&2
    return 1
  fi
  echo "ERROR: cannot $mode +i on $f (chattr denied; build ChickenBlockUpd: make bpf)" >&2
  return 1
}

for need in "$UNIT" "$ENV"; do
  if [ ! -e "$need" ]; then
    echo "Missing $need - install the ChickenBlocker systemd unit first (make install)." >&2
    exit 1
  fi
done

# Sanity: the installed ChickenBlocker.service MUST use WantedBy=ChickenBlocker.target
# (the new scheme). If it still says multi-user.target, the unit was not
# refreshed (run `make install` / `make install-core` first).
if ! grep -q '^WantedBy=ChickenBlocker.target$' "$UNIT"; then
  echo "ERROR: $UNIT does not contain 'WantedBy=ChickenBlocker.target'." >&2
  echo "       The installed unit is stale. Re-run 'make install' (or" >&2
  echo "       'make install-core') to install the updated ChickenBlocker.service" >&2
  echo "       and ChickenBlocker.target, then re-run this script." >&2
  exit 1
fi

# --- Layer 1: filesystem immutable -------------------------------------------
#
# Goal (ChickenBlocker-specific): block `systemctl disable ChickenBlocker` by making the
# ChickenBlocker.target.wants/ DIRECTORY immutable, so the kernel refuses to unlink
# the ChickenBlocker.service enable symlink inside it. That symlink stays a real
...
if [ ! -e "$TARGET" ]; then
  install -m0644 "$REPO_ROOT/ChickenBlocker.target" "$TARGET"
...
# multi-user.target.wants/ChickenBlocker.service). On a fresh install it is absent;
...
# shared multi-user.target.wants/ - NOT immutable, see ChickenBlocker.target docs).
if [ -L "$MU_TARGET_LINK" ] || [ -e "$MU_TARGET_LINK" ]; then
  echo "target enable symlink already present: $MU_TARGET_LINK"
else
  systemctl enable ChickenBlocker.target
  echo "enabled ChickenBlocker.target ($MU_TARGET_LINK)"
fi

# Enable the service (creates the ChickenBlocker.service symlink inside
...
if [ -L "$SERVICE_WANTS_LINK" ] || [ -e "$SERVICE_WANTS_LINK" ]; then
  ...
  systemctl enable ChickenBlocker.service
  echo "enabled ChickenBlocker.service ($SERVICE_WANTS_LINK)"
fi

# Make the unit file, target file, and the ChickenBlocker.target.wants/ DIRECTORY
# immutable. The directory immutability is what blocks `systemctl disable
# ChickenBlocker` (kernel EPERM on unlink of the service symlink inside it).
...
for f in "$UNIT" "$TARGET" "$TARGET_WANTS_DIR"; do
  ...
  if echo "$flags" | grep -q 'i'; then
    ...
    # chattr is confined by the chattr AppArmor profile (enforce) on ChickenBlocker
    # paths once that profile is loaded; on a FIRST install it isn't loaded yet
    # so chattr works. If chattr is denied, fall back to the gated ChickenBlockUpd
    ...
    elif [ -x "$REPO_ROOT/bpf/ChickenBlockUpd" ]; then
      "$REPO_ROOT/bpf/ChickenBlockUpd" set "$f"
      echo "set +i (immutable) on $f (via ChickenBlockUpd)"
    else
      echo "ERROR: could not set +i on $f (chattr denied; build ChickenBlockUpd: make bpf)" >&2
      exit 1
    fi
  fi
done
...
# New + existing protection profiles. The ChickenBlocker-related ones are also made
...
# ChickenBlocker profile files from /etc/apparmor.d/ (self-protection). Syntax-
...
for f in "${AA_PROFILES[@]}"; do
  ...
  if [ -f "$AA_DIR/$f" ] && lsattr "$AA_DIR/$f" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
    if [ -x "$REPO_ROOT/bpf/ChickenBlockUpd" ]; then
      "$REPO_ROOT/bpf/ChickenBlockUpd" clear "$AA_DIR/$f" || true
    else
  ...
  # profile, which denies reading the ChickenBlocker profile files from /etc/apparmor.d/.
...
for f in "${IMMUTABLE_PROFILES[@]}"; do
  if [ -f "$AA_DIR/$f" ] && ! lsattr "$AA_DIR/$f" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
    if [ -x "$REPO_ROOT/bpf/ChickenBlockUpd" ]; then
      "$REPO_ROOT/bpf/ChickenBlockUpd" set "$AA_DIR/$f" || true
    else
      chattr +i "$AA_DIR/$f" || true
    fi
...
# apparmor unit itself, complementing the ChickenBlocker unit's RefuseManualStop.
...
# cb_attr (chattr, else ChickenBlockUpd): if the override is +i and the BPF gate
...
# Added by ChickenBlocker: refuse manual stop so profiles can't be bulk-unloaded.
[Unit]
...
if [ -w /proc/sys/kernel/sysrq ]; then
  ...
  if ! grep -q '^kernel.sysrq' /etc/sysctl.d/99-ChickenBlocker.conf 2>/dev/null; then
    echo 'kernel.sysrq = 0' >> /etc/sysctl.d/99-ChickenBlocker.conf
  fi
...
# ChickenBlocker). This is the strongest userspace-accessible module protection.
# BPF programs are NOT modules, so the ChickenBlocker BPF LSM is unaffected.
#
...
# Only set it AFTER the ChickenBlocker BPF service is loaded + enabled (see bpf/).
if [ "${SET_MODULES_DISABLED:-0}" = "1" ]; then
  if [ "$(cat /proc/sys/kernel/modules_disabled 2>/dev/null || echo 0)" != "1" ]; then
    echo 1 > /proc/sys/kernel/modules_disabled
    if ! grep -q '^kernel.modules_disabled' /etc/sysctl.d/99-ChickenBlocker.conf 2>/dev/null; then
      echo 'kernel.modules_disabled = 1' >> /etc/sysctl.d/99-ChickenBlocker.conf
    fi
...
# ChickenBlocker-MANAGED browsers (firefox, vivaldi-stable) are EXCLUDED from the pin
...
# snap stubs: both show as "installed" but only firefox is managed by ChickenBlocker.
#
# Mechanism:
#   /etc/apt/preferences.d/99-ChickenBlocker-no-browsers pins the listed packages to
...
#   `make update` (which routes through the cap-endowed ChickenBlocker-update service,
...
#
# Protection of the pin file itself mirrors the ChickenBlocker unit files:
...
#     before the profile is reloaded, the gated ChickenBlockUpd clears +i).
#
...
if lsattr "$PREF_FILE" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
  ...
  if chattr +i "$PREF_FILE" 2>/dev/null; then
    ...
    "$REPO_ROOT/bpf/ChickenBlockUpd" set "$PREF_FILE"
    echo "set +i (immutable) on $PREF_FILE (via ChickenBlockUpd)"
  else
    echo "ERROR: could not set +i on $PREF_FILE (chattr denied; build ChickenBlockUpd: make bpf)" >&2
    exit 1
  fi
fi
...
echo "  systemctl is-enabled ChickenBlocker            # should say 'enabled'"
echo "  systemctl show -p Wants ChickenBlocker.target # should list ChickenBlocker.service"
echo "  sudo systemctl disable ChickenBlocker         # should fail: Operation not permitted"
...
echo
echo "RESIDUAL BYPASS (intentional): 'sudo systemctl disable ChickenBlocker.target'"
...
echo "  prevents boot-time start of ChickenBlocker on the NEXT reboot; the running"
echo "  persistent instance remains active. To close it globally:"
...
echo
echo "Build+install the C daemon (replaces main.py) BEFORE starting ChickenBlocker:"
echo "  cd src && make && sudo make install   # installs /opt/ChickenBlocker/ChickenBlocker"
