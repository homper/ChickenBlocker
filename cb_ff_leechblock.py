...
# the ChickenBlocker Firefox policies block). Mirrors the Vivaldi tool's name
...
# ~/.mozilla/firefox), mirroring ChickenBlocker's find_profile_path().
#
...
# /opt/ChickenBlocker. Its readable home is ~/snap/firefox/common, where it looks
...
# ChickenBlocker's find_profile_path(). Tries the snap layout first (where this
...
NOTES
    Firefox MUST be stopped for write subcommands (sqlite write lock + so
    Firefox does not overwrite the row on next sync). ChickenBlocker mirrors the
...
def main():
    ...
    if firefox_running():
        ...
        sys.exit("ERROR: firefox is running. Stop it first "
                 "(ChickenBlocker or `pkill firefox`) so it does not overwrite "
                 "the row, then re-run.")
    commit(con, d)
    con.close()
