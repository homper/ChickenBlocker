...
#
# This is a pragmatic bar matching the rest of the ChickenBlocker stack: hard to
...
# This script prefers the cap-endowed ChickenBlockUpd "set" subcommand when
...
UPDATER=""
for u in \
	"$(cd "$(dirname "$0")" && pwd)/bpf/ChickenBlockUpd" \
	"/opt/ChickenBlocker/bpf/ChickenBlockUpd"; do
	if [ -x "$u" ]; then
		UPDATER="$u"
		break
	fi
...
if [ -n "$UPDATER" ]; then
	# ChickenBlockUpd set <path> sets +i (CAP_LINUX_IMMUTABLE endowed via setcap
	# on the installed copy, or plain root pre-cap-drop on the repo build).
	if "$UPDATER" set "$PROFILES_INI" 2>/dev/null; then
		echo "locked (+i via ChickenBlockUpd): $PROFILES_INI"
		exit 0
	fi
