...
//
// cb-pause - interactive single-question client for ChickenBlocker's pause.
//
// Run in a terminal: /opt/ChickenBlocker/cb-pause
...
#define SOCK_NAME "ChickenBlocker-pause"   // abstract unix socket
#define QUESTIONS 5
...
int main(void) {
	...
	int s = dial();
	if (s < 0) { fprintf(stderr, "cb-pause: cannot reach the daemon (is ChickenBlocker running?).\n"); return 1; }
