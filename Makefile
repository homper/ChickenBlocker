...
#                       #   longer overwrite ChickenBlocker.service afterward)
...
install-armor: install-core
	...
	@echo "   can no longer overwrite ChickenBlocker.service because it is chattr +i)"
	...
	@# ChickenBlockUpd clear). The correct re-run path is `make update`, which
	@# delegates the +i work to the cap-endowed ChickenBlocker-update systemd service.
	@if sudo capsh --decode=$$(awk '/^CapBnd:/{print $$2}' /proc/self/status) 2>/dev/null \
		...
		printf '       to the cap-endowed ChickenBlocker-update systemd service.\n' >&2; \
		exit 1; \
	fi
	sudo ./apparmor/install.sh

install-bpf-boot: bpf
	@echo ">> Staging BPF artifacts -> /opt/ChickenBlocker/bpf/"
	sudo mkdir -p /opt/ChickenBlocker/bpf
	sudo chmod 0755 /opt/ChickenBlocker/bpf 2>/dev/null || true
	sudo install -m0755 bpf/ChickenBlocker_loader       /opt/ChickenBlocker/bpf/ChickenBlocker_loader
	sudo install -m0644 bpf/ChickenBlocker_lsm.bpf.o    /opt/ChickenBlocker/bpf/ChickenBlocker_lsm.bpf.o
	sudo install -m0755 bpf/ChickenBlockUpd        /opt/ChickenBlocker/bpf/ChickenBlockUpd
	@echo ">> Making BPF artifacts immutable"
	sudo chattr +i /opt/ChickenBlocker/bpf/ChickenBlocker_loader /opt/ChickenBlocker/bpf/ChickenBlocker_lsm.bpf.o /opt/ChickenBlocker/bpf
	@echo ">> Installing persistent ChickenBlocker-bpf.service (NOT enabled yet)"
	sudo install -m0644 bpf/ChickenBlocker-bpf.service /etc/systemd/system/ChickenBlocker-bpf.service
	@echo ">> Installing self-deleting firstboot -> /var/lib/ChickenBlocker/ChickenBlocker-bpf-boot"
	sudo install -d -m0700 /var/lib/ChickenBlocker
	sudo install -m0755 bpf/bpf-boot.sh /var/lib/ChickenBlocker/ChickenBlocker-bpf-boot
	@echo ">> Installing + enabling ChickenBlocker-bpf-firstboot.service"
	sudo install -m0644 bpf/ChickenBlocker-bpf-firstboot.service /etc/systemd/system/ChickenBlocker-bpf-firstboot.service
	sudo systemctl daemon-reload
	sudo systemctl enable ChickenBlocker-bpf-firstboot.service
	@echo
	...
	@echo "ChickenBlocker-bpf.service, then delete itself."
...
check: daemon
	@test -x src/ChickenBlocker && echo "daemon: ok"
	@$(MAKE) -C src check
	@bash -n configure-calendar.sh
	@python3 -m unittest tests/test_cb_calendar_sync.py tests/test_safeeyes_meeting.py
...
install-core: daemon
	...
	@missing=""; \
	command -v vdirsyncer >/dev/null 2>&1 || missing="$$missing vdirsyncer"; \
	command -v khal >/dev/null 2>&1 || missing="$$missing khal"; \
	python3 -c 'import aiohttp_oauthlib' >/dev/null 2>&1 \
		|| missing="$$missing python3-aiohttp-oauthlib"; \
	if [ -n "$$missing" ]; then \
		echo "Installing calendar tools (sudo apt):$$missing"; \
		sudo apt-get update -qq && sudo apt-get install -y $$missing \
			|| { echo "apt failed; run manually: sudo apt install$$missing"; exit 1; }; \
	fi
	@echo ">> Installing C daemon -> /opt/ChickenBlocker/ChickenBlocker"
	sudo $(MAKE) -C src install
	@echo ">> Installing source assets -> /opt/ChickenBlocker/ (daemon CWD)"
	@if sudo lsattr /opt/ChickenBlocker/policies.json 2>/dev/null | cut -c1-22 | grep -q 'i'; then \
		echo "SKIP: /opt/ChickenBlocker/policies.json is immutable (+i, locked); use 'make update' to refresh it."; \
	else \
		sudo install -m0644 policies.json /opt/ChickenBlocker/policies.json; \
	fi
	sudo install -m0644 policies-unblocked.json /opt/ChickenBlocker/policies-unblocked.json
	sudo install -m0644 userChrome.css /opt/ChickenBlocker/userChrome.css
	sudo install -m0644 userContent.css /opt/ChickenBlocker/userContent.css
	sudo install -m0644 vivaldi-policies.json /opt/ChickenBlocker/vivaldi-policies.json
	sudo install -m0644 vivaldi-policies-unblocked.json /opt/ChickenBlocker/vivaldi-policies-unblocked.json
	sudo install -m0755 cb_ff_activeblock.py /opt/ChickenBlocker/cb_ff_activeblock.py
	sudo install -m0755 cb_ff_leechblock.py /opt/ChickenBlocker/cb_ff_leechblock.py
	sudo install -m0755 cb_vv_leechblock.py /opt/ChickenBlocker/cb_vv_leechblock.py
	sudo install -m0755 cb_ff_clear_kwset.py /opt/ChickenBlocker/cb_ff_clear_kwset.py
	sudo install -m0644 safeeyes.json /opt/ChickenBlocker/safeeyes.json
	sudo install -m0644 safeeyes-night.json /opt/ChickenBlocker/safeeyes-night.json
	...
	@# can engage during a break). ChickenBlocker sets PYTHONPATH=/opt/ChickenBlocker so
	...
	@se_need_sync=0; \
	if [ ! -d /opt/ChickenBlocker/safeeyes ]; then se_need_sync=1; \
	else rsync -rn -c --out-format='%n' --exclude='__pycache__' \
		"$(CURDIR)/vendor/safeeyes/" /opt/ChickenBlocker/safeeyes/ 2>/dev/null \
		| grep -q . && se_need_sync=1; fi; \
	if [ "$$se_need_sync" = "1" ]; then \
		echo "   vendored copy changed (or first install): redeploying + quitting safeeyes"; \
		sudo rm -rf /opt/ChickenBlocker/safeeyes; \
		sudo cp -a vendor/safeeyes /opt/ChickenBlocker/safeeyes; \
		sudo find /opt/ChickenBlocker/safeeyes -name "__pycache__" -type d -prune -exec rm -rf {} +; \
		sudo find /opt/ChickenBlocker/safeeyes -type d -exec chmod 0755 {} +; \
		sudo find /opt/ChickenBlocker/safeeyes -type f -exec chmod 0644 {} +; \
		if pgrep -x safeeyes >/dev/null 2>&1; then \
			...
			if dbus-send --bus="$$dbsock" --print-reply --dest=org.freedesktop.DBus \
				...
				sudo -u "$$cbuser" DBUS_SESSION_BUS_ADDRESS="$$dbsock" \
					...
					DISPLAY=:0 PYTHONPATH=/opt/ChickenBlocker safeeyes -q 2>/dev/null || true; \
			fi; \
			for i in 1 2 3 4 5 6; do pgrep -x safeeyes >/dev/null 2>&1 || break; \
				pkill -TERM -x safeeyes 2>/dev/null || true; sleep 0.5; done; \
			echo "   safeeyes down; ChickenBlocker relaunches it (~30s) with PYTHONPATH=/opt/ChickenBlocker"; \
		fi; \
	else \
		echo "   vendored copy unchanged (already up to date); not touching safeeyes"; \
	fi
	sudo install -m0755 cb_av_check.py /opt/ChickenBlocker/cb_av_check.py
	sudo install -m0755 cb_break_check.py /opt/ChickenBlocker/cb_break_check.py
	sudo install -m0755 cb_calendar_sync.py /opt/ChickenBlocker/cb_calendar_sync.py
	sudo install -m0755 configure-calendar.sh /opt/ChickenBlocker/configure-calendar.sh
	...
	sed "s#\"install_url\": \"[^\"]*\"#\"install_url\": \"file:///etc/firefox/cb-extensions/page_keyword_filter.xpi\"#" policies.json \
		> /tmp/ChickenBlocker-policies.json && \
	sudo install -m0644 /tmp/ChickenBlocker-policies.json /etc/firefox/policies/policies.json && \
	rm -f /tmp/ChickenBlocker-policies.json
	...
	@if pgrep -x firefox >/dev/null 2>&1; then \
		...
		printf '    Stop firefox and run: sudo /opt/ChickenBlocker/cb_ff_clear_kwset.py\n' >&2; \
	else \
		sudo /opt/ChickenBlocker/cb_ff_clear_kwset.py || true; \
	fi
	@echo ">> Installing vivaldi-policies.json -> /etc/vivaldi/policies/managed/"
	sudo install -d -m0755 /etc/vivaldi/policies/managed
	sudo install -m0644 vivaldi-policies.json /etc/vivaldi/policies/managed/ChickenBlocker.json
	@echo ">> Installing systemd units -> /etc/systemd/system/"
	# ChickenBlocker.service and ChickenBlocker.target are both skipped if already
	# immutable (+i), i.e. after install-armor locked them. install.sh also
	# installs ChickenBlocker.target itself if missing, but we install it here too so
	# `make install` alone yields a working enabled setup without install-armor.
	@if sudo lsattr /etc/systemd/system/ChickenBlocker.service 2>/dev/null | cut -c1-22 | grep -q 'i'; then \
		echo "SKIP: /etc/systemd/system/ChickenBlocker.service is immutable (+i, locked by install-armor)"; \
		echo "      cannot update via make install; leaving the existing unit in place."; \
	else \
		sudo install -m0644 ChickenBlocker.service /etc/systemd/system/ChickenBlocker.service; \
	fi
	@if sudo lsattr /etc/systemd/system/ChickenBlocker.target 2>/dev/null | cut -c1-22 | grep -q 'i'; then \
		echo "SKIP: /etc/systemd/system/ChickenBlocker.target is immutable (+i, locked by install-armor)"; \
	else \
		sudo install -m0644 ChickenBlocker.target /etc/systemd/system/ChickenBlocker.target; \
	fi
	@echo ">> Ensuring /etc/default/ChickenBlocker exists (AppArmor guard needs it)"
	echo '# ChickenBlocker env (unused by the C daemon; kept for apparmor guard)' \
		| sudo tee /etc/default/ChickenBlocker >/dev/null
	sudo systemctl daemon-reload
	...
	# ChickenBlocker.target.wants/). After install-armor makes ChickenBlocker.target.wants/
	# immutable, `systemctl enable ChickenBlocker.service` cannot create the symlink,
	...
	@if [ -e /etc/systemd/system/multi-user.target.wants/ChickenBlocker.target ]; then \
		echo "ChickenBlocker.target enable symlink already present; skipping"; \
	else \
		sudo systemctl enable ChickenBlocker.target; \
	fi
	@if [ -e /etc/systemd/system/ChickenBlocker.target.wants/ChickenBlocker.service ]; then \
		echo "ChickenBlocker.service enable symlink already present; skipping"; \
	else \
		sudo systemctl enable ChickenBlocker.service; \
	fi
	...
	@systemctl is-active --quiet ChickenBlocker.service 2>/dev/null \
		|| sudo systemctl start ChickenBlocker.service
	@echo
...
install: install-core
	...
	@printf '  \033[1;36m3.\033[0m make install-update-service # cap-endowed ChickenBlocker-update service + setcap on updater\n'
...
update:
	@echo ">> Updating ChickenBlocker (build + refresh protected artifacts)"
	sudo ./update.sh

install-update-service:
	@echo ">> Installing cap-endowed ChickenBlocker-update service + setcap on updater"
...
# ChickenBlocker-update system service (installed here). Order enforced by make.
