#!/bin/bash
# Install the ChickenBlocker-update system service (the cap-endowed update channel)
# and grant CAP_LINUX_IMMUTABLE to the ChickenBlockUpd binary via file caps.
#
...
# terminal) lacks CAP_LINUX_IMMUTABLE, so it cannot run ChickenBlockUpd to clear
...
UPDATER_INSTALLED="$DEST/ChickenBlockUpd"
UPDATER_REPO="$REPO/bpf/ChickenBlockUpd"
...
# updater: the dir is immutable, so only ChickenBlockUpd can clear it), install
# the runner + unit, then re-+i the dir.
if [ -d "$DEST" ] && lsattr -d "$DEST" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
  "$UPDATER_REPO" clear "$DEST"
fi
install -m0755 "$HERE/ChickenBlocker-update-runner.sh" "$DEST/ChickenBlocker-update-runner.sh"
install -m0644 "$HERE/ChickenBlocker-update.service" /etc/systemd/system/ChickenBlocker-update.service
systemctl daemon-reload
if [ -d "$DEST" ] && ! lsattr -d "$DEST" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
  "$UPDATER_REPO" set "$DEST"
fi
# Do NOT enable: this is a triggered oneshot (no [Install]); it's started on
# demand by `systemctl start ChickenBlocker-update` from update.sh.
echo "installed ChickenBlocker-update.service (started on demand by update.sh)"
...
echo "  systemctl status ChickenBlocker-update.service"
