...
#   sudo rm /etc/systemd/system/lightdm.service.d/ChickenBlocker-cap-drop.conf \
#            /etc/systemd/system/getty@.service.d/ChickenBlocker-cap-drop.conf
...
install_dropin() {
  ...
  local file="$dir/ChickenBlocker-cap-drop.conf"
  mkdir -p "$dir"
  ...
  cat > "$file" <<EOF
# Added by ChickenBlocker: drop $CAP from this unit's bounding set so all sessions
...
echo
echo "RECOVERY:"
echo "  sudo rm /etc/systemd/system/lightdm.service.d/ChickenBlocker-cap-drop.conf \\"
echo "           /etc/systemd/system/getty@.service.d/ChickenBlocker-cap-drop.conf"
