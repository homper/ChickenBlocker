...
# shell at all (chattr -i, raw ioctl, etc.). Only the cap-endowed ChickenBlockUpd
# (run via ChickenBlocker-update.service) can.
#
...
#   sudo rm /etc/systemd/system/{user@,lightdm,getty@,autovt@}.service.d/ChickenBlocker-cap-drop.conf
...
install_cap_drop() {
  ...
  local dropfile="${dropdir}/ChickenBlocker-cap-drop.conf"

  mkdir -p "$dropdir"

  # Replace any prior version.
  cat > "$dropfile" <<'EOF'
# Added by ChickenBlocker: drop CAP_LINUX_IMMUTABLE from this unit so a shell
# (incl. sudo/su from the session) cannot clear the +i flag on protected
# files. Only the ChickenBlocker-update system service (and its cap-endowed updater)
...
echo
echo "RECOVERY: sudo rm /etc/systemd/system/{user@,lightdm,getty@,autovt@}.service.d/ChickenBlocker-cap-drop.conf && sudo systemctl daemon-reload  (then re-login)"
