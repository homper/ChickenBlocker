#!/bin/bash
# Build + install the ChickenBlocker BPF LSM signal blocker + the gated updater.
#
# Run as root, e.g.:  sudo ./install.sh
#
# What it does:
#   * verifies BPF LSM + BTF prerequisites
#   * builds the BPF object, the userspace loader, and the ChickenBlockUpd
#     gated +i setter/clearer (via make)
#   * copies artifacts to /opt/ChickenBlocker/bpf/
#   * installs + enables the ChickenBlocker-bpf.service systemd unit so the LSM
#     program is attached and pinned at every boot, before ChickenBlocker.service
...
#     tampering. The gated updater (comm "ChickenBlockUpd" (15 chars, comm == filename)) is the only thing
#     that can clear +i to refresh them — see update.sh.
#
# Once loaded, SIGHUP/SIGINT/SIGQUIT/SIGKILL/SIGUSR1/SIGUSR2/SIGSTOP sent to
# the ChickenBlocker process (comm == "ChickenBlocker", the C daemon) are denied even
# for root; SIGTERM remains available for managed shutdown and is force-restarted
# when externally delivered. The block_setflags_immutable program also denies
# clearing +i via raw
# FS_IOC_SETFLAGS ioctl (bypassing the chattr binary) except for the updater.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
DEST=/opt/ChickenBlocker/bpf
UNIT_SRC="$HERE/ChickenBlocker-bpf.service"
UNIT_DST=/etc/systemd/system/ChickenBlocker-bpf.service
...
if [ ! -x "$HERE/ChickenBlockUpd" ]; then
  echo "ERROR: ChickenBlockUpd did not build." >&2
  exit 1
fi
...
# via the repo-built ChickenBlockUpd (comm "ChickenBlockUpd" (15 chars, comm == filename) — the BPF gate
...
refresh_file() {
  ...
  if [ -e "$dst" ]; then
    "$HERE/ChickenBlockUpd" update "$dst" "$src"
  else
    cp "$src" "$dst"
    chmod "$mode" "$dst"
  fi
...
refresh_file "$DEST/ChickenBlocker_loader"    "$HERE/ChickenBlocker_loader"    0755
refresh_file "$DEST/ChickenBlocker_lsm.bpf.o" "$HERE/ChickenBlocker_lsm.bpf.o" 0644
refresh_file "$DEST/ChickenBlockUpd"     "$HERE/ChickenBlockUpd"     0755
...
"$DEST/ChickenBlocker_loader" "$DEST/ChickenBlocker_lsm.bpf.o"

systemctl enable --now ChickenBlocker-bpf.service
...
# ChickenBlockUpd is +i'd so it can't be replaced; it can still be refreshed by
# update.sh (which clears its +i via the gated repo updater, overwrites, re-+is).
# The ChickenBlocker daemon binary is +i'd too so a root shell cannot rm/overwrite
# it (the running process is kill-protected by the BPF signal blocker; the
# binary on disk is now also tamper-proof). update.sh refreshes it via the gated
# updater's ETXTBSY-aware atomic replace.
SET_LIST=()
for f in "$DEST/ChickenBlocker_loader" "$DEST/ChickenBlocker_lsm.bpf.o" "$DEST/ChickenBlockUpd" "/opt/ChickenBlocker/ChickenBlocker"; do
  if [ -e "$f" ] && ! lsattr "$f" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
    SET_LIST+=("$f")
  fi
done
if [ "${#SET_LIST[@]}" -gt 0 ]; then
  "$HERE/ChickenBlockUpd" set "${SET_LIST[@]}"
fi
...
if command -v setcap >/dev/null 2>&1; then
  setcap cap_mac_admin+ep "$DEST/ChickenBlockUpd" 2>/dev/null || true
fi
...
if [ -d "$DEST" ] && ! lsattr -d "$DEST" 2>/dev/null | cut -c1-22 | grep -q 'i'; then
  "$HERE/ChickenBlockUpd" set "$DEST"
fi

echo
echo "Done. Verify with:"
echo "  lsattr $DEST/ChickenBlocker_loader $DEST/ChickenBlocker_lsm.bpf.o $DEST   # all 'i'"
echo "  ls -l /sys/fs/bpf/ChickenBlocker_lsm /sys/fs/bpf/ChickenBlocker_setflags"
echo "  bpftool prog show | grep -E 'block_kill_ChickenBlocker|block_setflags_immutable'"
echo "  bpftool link show"
echo "  systemctl status ChickenBlocker-bpf.service"
echo
echo "Test (the following should now fail with EPERM, even as root):"
echo "  sudo kill -9 \$(pgrep -x ChickenBlocker)"
echo "  sudo chattr -i $DEST/ChickenBlocker_loader     # denied by BPF (not gated updater)"
echo "  sudo rm $DEST/ChickenBlocker_loader            # denied: dir is +i"
echo
