...
# (or from the repo: bpf/ChickenBlocker_loader bpf/ChickenBlocker_lsm.bpf.o)
#
...
TESTFILE="$(mktemp /tmp/ChickenBlocker-setflags-test.XXXXXX)"
...
# ChickenBlockUpd: the gated updater (comm "ChickenBlockUpd" (15 chars, comm == filename)) whose clear
#                  attempt MUST be ALLOWED by the BPF gate (positive test).
UPDATER="$HERE/../ChickenBlockUpd"
if [ ! -x "$HELPER" ]; then
  echo "Building ioctl_clear helper ..."
  cc -O2 -Wall "$HERE/ioctl_clear.c" -o "$HELPER" || { echo "build failed" >&2; exit 2; }
fi
if [ ! -x "$UPDATER" ]; then
  echo "Building ChickenBlockUpd ..."
  cc -O2 -Wall "$HERE/../ChickenBlockUpd.c" -o "$UPDATER" || { echo "updater build failed" >&2; exit 2; }
fi
...
echo "=== BPF load state ==="
ls -l /sys/fs/bpf/ChickenBlocker_lsm /sys/fs/bpf/ChickenBlocker_setflags 2>/dev/null || true
bpftool prog show 2>/dev/null | grep -E 'block_kill_ChickenBlocker|block_setflags_immutable' || true
...
if [ ! -e /sys/fs/bpf/ChickenBlocker_setflags ] || \
  ...
  echo "       Only block_kill_ChickenBlocker is loaded (that's a different program)." >&2
  echo "       Load both programs first, e.g.:" >&2
  echo "         sudo bpf/install.sh" >&2
  echo "       (or: sudo bpf/ChickenBlocker_loader bpf/ChickenBlocker_lsm.bpf.o)" >&2
...
if chattr -i "$TESTFILE" 2>/dev/null; then
  echo "control: chattr -i SUCCEEDED (expected: /tmp is not a ChickenBlocker path, so the chattr profile does not block it)."
else
  echo "control: chattr -i DENIED (AppArmor chattr profile blocking — expected on ChickenBlocker paths)."
fi
...
if "$UPDATER" clear "$TESTFILE"; then
  echo "RESULT: updater CLEARED +i — BPF gate allows ChickenBlockUpd (update mechanism OK)."
  UPDATER_OK=0
else
  echo "RESULT: updater DENIED — gate did NOT allow ChickenBlockUpd (update mechanism BROKEN)."
  UPDATER_OK=1
fi
