...
BPF="$HERE/.."
OBJ="$BPF/ChickenBlocker_lsm.bpf.o"
LOADER="$BPF/ChickenBlocker_loader"
HELPER="$HERE/ioctl_clear"
UPDATER="$BPF/ChickenBlockUpd"
...
if [ ! -f "$OBJ" ]; then
  echo "ERROR: $OBJ missing. Build it as YOUR user (not root) first: make -C $BPF ChickenBlocker_lsm.bpf.o" >&2
  exit 2
fi
[ -x "$HELPER" ]  || cc -O2 -Wall "$HERE/ioctl_clear.c" -o "$HELPER"
[ -x "$UPDATER" ] || cc -O2 -Wall "$BPF/ChickenBlockUpd.c" -o "$UPDATER"
...
rm -f /sys/fs/bpf/ChickenBlocker_setflags
"$LOADER" "$OBJ"
