...
/*
 * Userspace loader for the ChickenBlocker BPF LSM programs.
 *
...
#ifndef BPF_OBJ_PATH
#define BPF_OBJ_PATH "/opt/ChickenBlocker/bpf/ChickenBlocker_lsm.bpf.o"
#endif
 ...
 * backward compatibility; new programs get /sys/fs/bpf/ChickenBlocker_<name>. */
static const char *pin_path_for(const char *prog_name)
{
	if (strcmp(prog_name, "block_kill_ChickenBlocker") == 0)
		return "/sys/fs/bpf/ChickenBlocker_lsm";
	if (strcmp(prog_name, "block_setflags_immutable") == 0)
		return "/sys/fs/bpf/ChickenBlocker_setflags";
	/* Fallback for future programs. */
	static char buf[256];
	snprintf(buf, sizeof(buf), "/sys/fs/bpf/ChickenBlocker_%s", prog_name);
	return buf;
}
...
int main(int argc, char **argv)
{
	...
	bpf_object__for_each_program(prog, obj) {
		...
		if (access(pin, F_OK) == 0) {
			printf("ChickenBlocker: %s already pinned at %s; skipping\n", name, pin);
			continue;
		}
		...
		printf("ChickenBlocker: %s attached and pinned at %s\n", name, pin);
	...
	if (attached == 0) {
		printf("ChickenBlocker: all programs already pinned; nothing to do.\n");
	}
	return 0;
}
