...
/*
 * ChickenBlockUpd — the gated +i manager.
 *
 * Binary name is "ChickenBlockUpd" (exactly 15 chars), which fits the kernel
 * task comm cap (TASK_COMM_LEN=16 incl. NUL = 15 usable chars), so on exec
 * the comm == the filename with NO truncation. The block_setflags_immutable
 * BPF LSM program allows FS_IOC_SETFLAGS on IMMUTABLE inodes (i.e. clearing
 * +i) ONLY when the caller's comm matches UPDATE_COMM (="ChickenBlockUpd")
 * — i.e. only this program. So only this binary can clear +i on protected
 * files; a normal root shell cannot (chattr -i is denied by AppArmor; raw
 * ioctl is denied by the BPF gate).
 *
 * Modes:
 *   ChickenBlockUpd set    <file>...          set +i (FS_IMMUTABLE_FL).
 ...
 *   ChickenBlockUpd clear  <file>...          clear +i (GATED — only this comm).
 *                         Used by install.sh re-runs and diagnostics.
 *   ChickenBlockUpd update <dst> <src>        ATOMIC refresh: clear +i on dst
 ...
 * ChickenBlockUpd). The planned cap-drop (pam_cap + file capabilities) makes
 ...
 * fails with ETXTBSY — i.e. dst is a RUNNING executable (the live ChickenBlocker
...
static void usage(void)
{
	fprintf(stderr,
		"Usage:\n"
		"  ChickenBlockUpd set    <file>...           # set +i\n"
		"  ChickenBlockUpd clear  <file>...           # clear +i (gated)\n"
		"  ChickenBlockUpd update <dst> <src>         # atomic: clear+copy+re-+i\n");
	exit(2);
}
