...
/*
 * BPF LSM programs for ChickenBlocker protection.
 *
 * Program 1: block_kill_ChickenBlocker (lsm/task_kill)
 *   Deny SIGHUP/SIGINT/SIGQUIT/SIGKILL/SIGUSR1/SIGUSR2/SIGSTOP to the
 *   ChickenBlocker daemon (comm == "ChickenBlocker"). SIGTERM remains allowed for
 *   managed shutdown; the service force-restarts externally delivered SIGTERM.
 *   This is the only userspace-accessible mechanism that truly blocks root
 *   SIGKILL (AppArmor cannot).
 *
...
static const char MATCH_COMM[16] = "ChickenBlocker";
 ...
 * task's comm matches this. The updater binary is named "ChickenBlockUpd"
 * (exactly 15 chars), which fits the kernel task comm cap (TASK_COMM_LEN=16
 * incl. NUL = 15 usable chars), so the comm == the filename exactly with NO
 * truncation. UPDATE_COMM matches that 15-char runtime comm. If you rename the
 * updater binary, keep its name <= 15 chars and update this constant to match,
 * then rebuild+reload. */
static const char UPDATE_COMM[16] = "ChickenBlockUpd";
...
SEC("lsm/task_kill")
int BPF_PROG(block_kill_ChickenBlocker, struct task_struct *target,
