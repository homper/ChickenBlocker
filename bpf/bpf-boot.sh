#!/bin/bash
# ChickenBlocker-bpf firstboot installer.
#
# Installed to an obscure system path (see Makefile: install-bpf-boot) and run
# once at boot via ChickenBlocker-bpf-firstboot.service. It:
...
#   3. enables the PERSISTENT ChickenBlocker-bpf.service so every later boot
...
# /opt/ChickenBlocker/bpf/. Nothing here builds anything - the toolchain is not
# needed at boot.
#
...
LOADER=/opt/ChickenBlocker/bpf/ChickenBlocker_loader
OBJ=/opt/ChickenBlocker/bpf/ChickenBlocker_lsm.bpf.o
PERSIST_UNIT=ChickenBlocker-bpf.service
PERSIST_FILE=/etc/systemd/system/$PERSIST_UNIT
SELF_UNIT=ChickenBlocker-bpf-firstboot.service
SELF_FILE=/etc/systemd/system/$SELF_UNIT
LOG=/var/log/ChickenBlocker-bpf-firstboot.log
...
# failure (e.g. read-only mount). The script lives under /var/lib/ChickenBlocker
